...
val_to_date(t_val& item, t_date* out) {
    ...
    if (item.typeOf().as<std::string>().compare("string") == 0) {
        tt = time_t(
            apachearrow::parseAsArrowTimestamp(item.as<std::string>()) / 1000);
...
vector_to_typed_array(std::vector<T>& xs) {
    T* st = &xs[0];
    auto offset = reinterpret_cast<uintptr_t>(st);
...
str_to_arraybuffer(std::shared_ptr<std::string> str) {
    char* start = &(*str)[0];
    auto offset = reinterpret_cast<uintptr_t>(start);
    return t_val::module_property("HEAPU8").call<t_val>(
...
namespace arraybuffer {
    ...
    template <>
    void
    vecFromTypedArray(const t_val& typedArray, void* data, std::int32_t length,
        const char* destType) {
    ...
    template <>
    void
    fill_col_valid(t_val dcol, const std::shared_ptr<t_column>& col) {
    ...
    template <>
    void
    fill_col_dict(const t_val& dictvec, const std::shared_ptr<t_column>& col) {
        ...
        auto vsize = vdata["length"].as<std::int32_t>();
        ...
        auto osize = voffsets["length"].as<std::int32_t>();
        ...
        auto dsize = dictvec["length"].as<std::uint32_t>();
...
col_to_typed_array<bool>(const std::vector<t_tscalar>& data) {
    ...
    t_val arr = t_val::global("Array").new_();
    arr.call<void>("push",
        typed_array<std::int8_t>.new_(vector_to_typed_array(vals)["buffer"]));
    arr.call<void>("push", nullCount);
    arr.call<void>("push", vector_to_typed_array(validityMap));
    return arr;
}
...
std::vector<t_dtype>
get_data_types(
    t_val data, std::int32_t format, const std::vector<std::string>& names) {
    if (names.size() == 0) {
...
void
_fill_col_time(t_data_accessor accessor, const std::shared_ptr<t_column>& col,
...
void
_fill_col_date(t_data_accessor accessor, const std::shared_ptr<t_column>& col,
    const std::string& name, std::int32_t cidx, t_dtype type, bool is_update,
    bool is_limit) {
    t_uindex nrows = col->size();
    for (auto i = 0; i < nrows; ++i) {
...
void
_fill_col_bool(t_data_accessor accessor, const std::shared_ptr<t_column>& col,
    const std::string& name, std::int32_t cidx, t_dtype type, bool is_update,
    bool is_limit) {
    t_uindex nrows = col->size();
    for (auto i = 0; i < nrows; ++i) {
...
void
_fill_col_string(t_data_accessor accessor, const std::shared_ptr<t_column>& col,
    const std::string& name, std::int32_t cidx, t_dtype type, bool is_update,
    bool is_limit) {
    t_uindex nrows = col->size();
    for (auto i = 0; i < nrows; ++i) {
...
_fill_col_int64(t_data_accessor accessor, t_data_table& tbl,
    std::shared_ptr<t_column> col, const std::string& name, std::int32_t cidx,
...
void
set_column_nth(
    const std::shared_ptr<t_column>& col, t_uindex idx, const t_val& value) {
    ...
    switch (col->get_dtype()) {
        ...
        case DTYPE_TIME: {
            auto elem = static_cast<std::int64_t>(
                value.call<t_val>("getTime").as<double>()); // dcol[i].as<T>();
...
_fill_data(t_data_table& tbl, t_data_accessor dcol,
    ...
    for (auto cidx = 0; cidx < col_names.size(); ++cidx) {
        ...
        if (name == "__INDEX__") {
            ...
            _fill_data_helper(dcol, tbl, pkey_col_sptr, "psp_pkey", cidx, type,
                is_update, is_limit);
...
make_table(t_val table, t_data_accessor accessor, std::uint32_t limit,
    ...
    if (is_arrow && !is_delete) {
        if (is_csv) {
                    ...
                    switch (type) {
                        ...
                        default:
                            ...
                            ss << "Error loading arrow type "
                               << dtype_to_str(type) << " for column " << name
                               << std::endl;
                            PSP_COMPLAIN_AND_ABORT(ss.str())
                            break;
                    }
            ...
            auto length = accessor["byteLength"].as<std::int32_t>();
        ...
        auto format = accessor["format"].as<std::int32_t>();
...
is_valid_filter(t_dtype column_type, t_val date_parser,
    ...
    if (column_type == DTYPE_DATE || column_type == DTYPE_TIME) {
...
make_view_config(const t_gnode& gnode, std::shared_ptr<t_schema> schema,
    ...
    auto column_pivots = config.call<std::vector<std::string>>("get_split_by");
    ...
    for (const auto& expr : js_expressions) {
        auto expression_alias = expr[0].as<std::string>();
        auto expression_string = expr[1].as<std::string>();
        auto parsed_expression_string = expr[2].as<std::string>();
    ...
    for (auto f : js_filter) {
        ...
        auto column_name = f.at(0).as<std::string>();
        auto filter_op_str = f.at(1).as<std::string>();
    ...
    auto view_config
        = std::make_shared<t_view_config>(row_pivots, column_pivots, aggregates,
            columns, filter, sort, expressions, filter_op, column_only);
...
make_view(std::shared_ptr<Table> table, const std::string& name,
    ...
    auto view_ptr
        = std::make_shared<View<CTX_T>>(table, ctx, name, separator, config);
