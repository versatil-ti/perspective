...
const std::string months_of_year[12] = {"01 January", "02 February", "03 March",
...
index::index(std::shared_ptr<t_data_table> source_table, t_uindex& row_idx)
    : exprtk::igeneric_function<t_tscalar>("Z")
    , m_source_table(std::move(source_table))
    , m_row_idx(row_idx) {}
...
col::col(std::shared_ptr<t_data_table> source_table, t_uindex& row_idx)
    : exprtk::igeneric_function<t_tscalar>("T")
    , m_source_table(std::move(source_table))
    , m_row_idx(row_idx) {}
