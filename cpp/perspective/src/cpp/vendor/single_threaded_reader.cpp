...
// This file is a fork of `arrow/ipc/reader.cc`
// this file removes a usage of threading so it is compatible with WASM.
...
namespace ipc {
    ...
    namespace {
        ...
        Status
        InvalidMessageType(MessageType expected, MessageType actual) {
            return Status::IOError("Expected IPC message of type ",
                FormatMessageType(expected), " but got ",
                FormatMessageType(actual));
        }
...
#define CHECK_HAS_NO_BODY(message)                                             \
    ...
    struct IpcReadContext {
        IpcReadContext(DictionaryMemo* memo, const IpcReadOptions& option,
            bool swap, MetadataVersion version = MetadataVersion::V5,
        ...
        /// \brief LoadRecordBatch() or LoadRecordBatchSubset() swaps endianness
        /// of elements if this flag is true
        const bool swap_endian;
    };

    /// A collection of ranges to read and pointers to set to those ranges when
    /// they are available.  This allows the ArrayLoader to utilize a two pass
    /// cache-then-read strategy with a ReadRangeCache
    class BatchDataReadRequest {
    public:
        [[nodiscard]] const std::vector<io::ReadRange>&
        ranges_to_read() const {
            return ranges_to_read_;
        }

        void
        RequestRange(
            int64_t offset, int64_t length, std::shared_ptr<Buffer>* out) {
            ranges_to_read_.push_back({offset, length});
            destinations_.push_back(out);
        }

        void
        FulfillRequest(const std::vector<std::shared_ptr<Buffer>>& buffers) {
    ...
    /// The field_index and buffer_index are incremented based on how much of
    /// the batch is "consumed" (through nested data reconstruction, for
    /// example)
    class ArrayLoader {
    public:
        ...
        ReadBuffer(
            ...
            if (offset < 0) {
                return Status::Invalid(
                    "Negative offset for reading buffer ", buffer_index_);
            }
            if (length < 0) {
                return Status::Invalid(
                    "Negative length for reading buffer ", buffer_index_);
            }
        ...
        Status
        LoadType(const DataType& type) {
            return VisitTypeInline(type, this);
        }

        Status
        Load(const Field* field, ArrayData* out) {
        ...
        Status
        GetBuffer(int buffer_index, std::shared_ptr<Buffer>* out) {
            const auto* buffers = metadata_->buffers();
        ...
        Status
        GetFieldMetadata(int field_index, ArrayData* out) {
            const auto* nodes = metadata_->nodes();
        ...
        Status
        LoadCommon(Type::type type_id) {
            // This only contains the length and null count, which we need to
            // figure out what to do with the buffers. For example, if
            // null_count == 0, then we can skip that buffer without reading
            // from shared memory
            RETURN_NOT_OK(GetFieldMetadata(field_index_++, out_));

            if (internal::HasValidityBitmap(type_id, metadata_version_)) {
                // Extract null_bitmap which is common to all arrays except for
                // unions and nulls.
                if (out_->null_count != 0) {
                    RETURN_NOT_OK(
                        GetBuffer(buffer_index_, out_->buffers.data()));
                }
                buffer_index_++;
            }
            return Status::OK();
        ...
        Status
        LoadPrimitive(Type::type type_id) {
        ...
        LoadList(const TYPE& type) {
            ...
            if (num_children != 1) {
                return Status::Invalid(
                    "Wrong number of children: ", num_children);
            }
        ...
        Status
        LoadChildren(const std::vector<std::shared_ptr<Field>>& child_fields) {
            ...
            for (int i = 0; i < static_cast<int>(child_fields.size()); ++i) {
                ...
                RETURN_NOT_OK(
                    Load(child_fields[i].get(), parent->child_data[i].get()));
                ++max_recursion_depth_;
            }
            out_ = parent;
            return Status::OK();
        ...
        template <typename T>
        enable_if_base_binary<T, Status>
        Visit(const T& type) {
            return LoadBinary<T>(type.id());
        }

        Status
        Visit(const FixedSizeBinaryType& type) {
            out_->buffers.resize(2);
            RETURN_NOT_OK(LoadCommon(type.id()));
        ...
        Visit(const UnionType& type) {
            ...
            // Trying to fix up union data to do without the top-level validity
            // bitmap is hairy:
            // - type ids must be rewritten to all have valid values (even for
            // former
            //   null slots)
            // - sparse union children must have their validity bitmaps
            // rewritten
            ...
            if (out_->null_count != 0 && out_->buffers[0] != nullptr) {
                return Status::Invalid("Cannot read pre-1.0.0 Union array with "
            ...
            if (out_->length > 0) {
                ...
                if (type.mode() == UnionMode::DENSE) {
                    RETURN_NOT_OK(
                        GetBuffer(buffer_index_ + 1, &out_->buffers[2]));
                }
            }
        ...
        Status
        Visit(const DictionaryType& type) {
    ...
    DecompressBuffer(const std::shared_ptr<Buffer>& buf,
        const IpcReadOptions& options, util::Codec* codec) {
        ...
        ARROW_ASSIGN_OR_RAISE(int64_t actual_decompressed,
    ...
    Status
    DecompressBuffers(Compression::type compression,
        const IpcReadOptions& options, ArrayDataVector* fields) {
        struct BufferAccumulator {
            using BufferPtrVector = std::vector<std::shared_ptr<Buffer>*>;

            void
            AppendFrom(const ArrayDataVector& fields) {
            ...
            BufferPtrVector
            Get(const ArrayDataVector& fields) && {
                AppendFrom(fields);
        ...
        // PSP_PARALLEL_FOR works in wasm.
        // TODO: THIS ONE RIGHT HERE!

        for (auto& buffer : buffers) {
            ARROW_ASSIGN_OR_RAISE(
                *buffer, DecompressBuffer(*buffer, options, codec.get()));
        }
        return Status::OK();
    }

    Result<std::shared_ptr<RecordBatch>>
    LoadRecordBatchSubset(const flatbuf::RecordBatch* metadata,
        const std::shared_ptr<Schema>& schema,
        const std::vector<bool>* inclusion_mask, const IpcReadContext& context,
        io::RandomAccessFile* file) {
        ArrayLoader loader(
            metadata, context.metadata_version, context.options, file);
        ...
        for (int i = 0; i < schema->num_fields(); ++i) {
            ...
            if ((inclusion_mask == nullptr) || (*inclusion_mask)[i]) {
                // Read field
                ...
                if (inclusion_mask != nullptr) {
                    filtered_columns.push_back(columns[i]);
                    filtered_fields.push_back(schema->field(i));
                }
            } else {
                // Skip field. This logic must be executed to advance the state
                // of the loader to the next field
                RETURN_NOT_OK(loader.SkipField(&field));
            }
        }

        // Dictionary resolution needs to happen on the unfiltered columns,
        // because fields are mapped structurally (by path in the original
        // schema).
        RETURN_NOT_OK(ResolveDictionaries(
            columns, *context.dictionary_memo, context.options.memory_pool));

        if (inclusion_mask != nullptr) {
            filtered_schema = ::arrow::schema(
                std::move(filtered_fields), schema->metadata());
            columns.clear();
        } else {
        ...
        if (context.compression != Compression::UNCOMPRESSED) {
            RETURN_NOT_OK(DecompressBuffers(
                context.compression, context.options, &filtered_columns));
        }
    ...
    Result<std::shared_ptr<RecordBatch>>
    LoadRecordBatch(const flatbuf::RecordBatch* metadata,
        const std::shared_ptr<Schema>& schema,
        const std::vector<bool>& inclusion_mask, const IpcReadContext& context,
        io::RandomAccessFile* file) {
        if (!inclusion_mask.empty()) {
            return LoadRecordBatchSubset(
                metadata, schema, &inclusion_mask, context, file);
        }
        return LoadRecordBatchSubset(
            metadata, schema, /*inclusion_mask=*/nullptr, context, file);
    }
    ...
    Status
    GetCompression(const flatbuf::RecordBatch* batch, Compression::type* out) {
        *out = Compression::UNCOMPRESSED;
        ...
        if (compression != nullptr) {
            if (compression->method()
                ...
                return Status::Invalid(
                    "This library only supports BUFFER compression method");
            }
            ...
            if (compression->codec() == flatbuf::CompressionType::LZ4_FRAME) {
                ...
                *out = Compression::ZSTD;
                ...
                return Status::Invalid(
                    "Unsupported codec in RecordBatch::compression metadata");
            }
            return Status::OK();
        }
        return Status::OK();
    ...
    Status
    GetCompressionExperimental(
        const flatbuf::Message* message, Compression::type* out) {
        *out = Compression::UNCOMPRESSED;
        if (message->custom_metadata() != nullptr) {
            // TODO: Ensure this deserialization only ever happens once
            std::shared_ptr<KeyValueMetadata> metadata;
            RETURN_NOT_OK(internal::GetKeyValueMetadata(
                message->custom_metadata(), &metadata));
            int index = metadata->FindKey("ARROW:experimental_compression");
            if (index != -1) {
                // Arrow 0.17 stored string in upper case, internal utils now
                // require lower case
                auto name
                    = arrow::internal::AsciiToLower(metadata->value(index));
                ARROW_ASSIGN_OR_RAISE(
                    *out, util::Codec::GetCompressionType(name));
            }
    ...
    static Status
    ReadContiguousPayload(
        io::InputStream* file, std::unique_ptr<Message>* message) {
    ...
    Result<std::shared_ptr<RecordBatch>>
    ReadRecordBatch(const std::shared_ptr<Schema>& schema,
    ...
    Result<std::shared_ptr<RecordBatch>>
    ReadRecordBatch(const Message& message,
        const std::shared_ptr<Schema>& schema,
        const DictionaryMemo* dictionary_memo, const IpcReadOptions& options) {
    ...
    Result<RecordBatchWithMetadata>
    ReadRecordBatchInternal(const Buffer& metadata,
        const std::shared_ptr<Schema>& schema,
        const std::vector<bool>& inclusion_mask, IpcReadContext& context,
        io::RandomAccessFile* file) {
        const flatbuf::Message* message = nullptr;
        RETURN_NOT_OK(internal::VerifyMessage(
            metadata.data(), metadata.size(), &message));
        const auto* batch = message->header_as_RecordBatch();
        if (batch == nullptr) {
            return Status::IOError("Header-type of flatbuffer-encoded Message "
                                   "is not RecordBatch.");
        }
        ...
        if (context.compression == Compression::UNCOMPRESSED
            ...
            // Possibly obtain codec information from experimental serialization
            // format in 0.17.x
        ...
        if (message->custom_metadata() != nullptr) {
            RETURN_NOT_OK(internal::GetKeyValueMetadata(
                message->custom_metadata(), &custom_metadata));
        }
    ...
    // If we are selecting only certain fields, populate an inclusion mask for
    // fast lookups. Additionally, drop deselected fields from the reader's
    // schema.
    Status
    GetInclusionMaskAndOutSchema(const std::shared_ptr<Schema>& full_schema,
        ...
        std::sort(
            included_indices_sorted.begin(), included_indices_sorted.end());
        ...
        *out_schema = schema(std::move(included_fields),
            full_schema->endianness(), full_schema->metadata());
        return Status::OK();
    }

    Status
    UnpackSchemaMessage(const void* opaque_schema,
        const IpcReadOptions& options, DictionaryMemo* dictionary_memo,
        std::shared_ptr<Schema>* schema, std::shared_ptr<Schema>* out_schema,
        std::vector<bool>* field_inclusion_mask, bool* swap_endian) {
        RETURN_NOT_OK(
            internal::GetSchema(opaque_schema, dictionary_memo, schema));

        // If we are selecting only certain fields, populate the inclusion mask
        // now for fast lookups
        RETURN_NOT_OK(GetInclusionMaskAndOutSchema(*schema,
            options.included_fields, field_inclusion_mask, out_schema));
        *swap_endian = options.ensure_native_endian
            && !out_schema->get()->is_native_endian();
        if (*swap_endian) {
            // create a new schema with native endianness before swapping endian
            // in ArrayData
    ...
    Status
    UnpackSchemaMessage(const Message& message, const IpcReadOptions& options,
        ...
        return UnpackSchemaMessage(message.header(), options, dictionary_memo,
            schema, out_schema, field_inclusion_mask, swap_endian);
    }

    Result<std::shared_ptr<RecordBatch>>
    ReadRecordBatch(const Buffer& metadata,
        const std::shared_ptr<Schema>& schema,
        const DictionaryMemo* dictionary_memo, const IpcReadOptions& options,
        io::RandomAccessFile* file) {
        std::shared_ptr<Schema> out_schema;
        ...
        IpcReadContext context(
            const_cast<DictionaryMemo*>(dictionary_memo), options, false);
        RETURN_NOT_OK(GetInclusionMaskAndOutSchema(schema,
            context.options.included_fields, &inclusion_mask, &out_schema));
        ARROW_ASSIGN_OR_RAISE(auto batch_and_custom_metadata,
    ...
    Status
    ReadDictionary(const Buffer& metadata, const IpcReadContext& context,
        DictionaryKind* kind, io::RandomAccessFile* file) {
        const flatbuf::Message* message = nullptr;
        RETURN_NOT_OK(internal::VerifyMessage(
            metadata.data(), metadata.size(), &message));
        const auto* const dictionary_batch
            = message->header_as_DictionaryBatch();
        if (dictionary_batch == nullptr) {
            return Status::IOError("Header-type of flatbuffer-encoded Message "
                                   "is not DictionaryBatch.");
        }

        // The dictionary is embedded in a record batch with a single column
        const auto* const batch_meta = dictionary_batch->data();
        ...
        ARROW_ASSIGN_OR_RAISE(
            auto value_type, context.dictionary_memo->GetDictionaryType(id));

        // Load the dictionary data from the dictionary batch
        ArrayLoader loader(batch_meta,
            internal::GetMetadataVersion(message->version()), context.options,
            file);
        ...
        if (compression != Compression::UNCOMPRESSED) {
            ...
            RETURN_NOT_OK(
                DecompressBuffers(compression, context.options, &dict_fields));
        }

        // swap endian in dict_data if necessary (swap_endian == true)
        if (context.swap_endian) {
            ARROW_ASSIGN_OR_RAISE(
                dict_data, ::arrow::internal::SwapEndianArrayData(dict_data));
        }
        ...
        if (kind != nullptr) {
            *kind
                = inserted ? DictionaryKind::New : DictionaryKind::Replacement;
        }
        return Status::OK();
    }

    Status
    ReadDictionary(const Message& message, const IpcReadContext& context,
        DictionaryKind* kind) {
        // Only invoke this method if we already know we have a dictionary
        // message
    ...
    public:
        Status
        Open(std::unique_ptr<MessageReader> message_reader,
            const IpcReadOptions& options) {
            ...
            // Read schema
            ARROW_ASSIGN_OR_RAISE(
                std::unique_ptr<Message> message, ReadNextMessage());
            if (!message) {
                return Status::Invalid(
                    "Tried reading schema message, was null or length 0");
            }

            RETURN_NOT_OK(UnpackSchemaMessage(*message, options,
        ...
        Status
        ReadNext(std::shared_ptr<RecordBatch>* batch) override {
        ...
        ReadNext() override {
            ...
            if (empty_stream_) {
                // ARROW-6006: Degenerate case where stream contains no data, we
                // do not bother trying to read a RecordBatch message from the
                // stream
            ...
            while (message != nullptr
                && message->type() == MessageType::DICTIONARY_BATCH) {
            ...
            CHECK_HAS_BODY(*message);
            ARROW_ASSIGN_OR_RAISE(
                auto reader, Buffer::GetReader(message->body()));
        ...
        [[nodiscard]] std::shared_ptr<Schema>
    ...
    private:
        ...
        ReadNextMessage() {
            ARROW_ASSIGN_OR_RAISE(
                auto message, message_reader_->ReadNextMessage());
            if (message) {
                ++stats_.num_messages;
                switch (message->type()) {
        ...
        Status
        ReadDictionary(const Message& message) {
            DictionaryKind kind;
            IpcReadContext context(&dictionary_memo_, options_, swap_endian_);
            RETURN_NOT_OK(
                ::arrow::ipc::ReadDictionary(message, context, &kind));
            switch (kind) {
        ...
        ReadInitialDictionaries() {
            ...
            // TODO(wesm): In future, we may want to reconcile the ids in the
            // stream with those found in the schema
            const auto num_dicts = dictionary_memo_.fields().num_dicts();
            for (int i = 0; i < num_dicts; ++i) {
                ARROW_ASSIGN_OR_RAISE(message, ReadNextMessage());
                if (!message) {
                    if (i == 0) {
                        /// ARROW-6006: If we fail to find any dictionaries in
                        /// the stream, then it may be that the stream has a
                        /// schema but no actual data. In such case we
                        /// communicate that we were unable to find the
                        /// dictionaries (but there was no failure otherwise),
                        /// so the caller can decide what to do
                        empty_stream_ = true;
                        break;
                    } // ARROW-6126, the stream terminated before receiving
                    // the expected number of dictionaries
                    return Status::Invalid("IPC stream ended without "
                                           "reading the expected number (",
                        num_dicts, ") of dictionaries");
                }

                if (message->type() != MessageType::DICTIONARY_BATCH) {
                    return Status::Invalid(
                        "IPC stream did not have the expected number (",
                        num_dicts,
        ...
        // Flag to set in case where we fail to observe all dictionaries in a
        // stream, and so the reader should not attempt to parse any messages
    ...
    Result<std::shared_ptr<RecordBatchStreamReader>>
    RecordBatchStreamReader::Open(std::unique_ptr<MessageReader> message_reader,
        const IpcReadOptions& options) {
        // Private ctor
    ...
    Status
    CheckAligned(const FileBlock& block) {
    ...
    static Result<std::unique_ptr<Message>>
    ReadMessageFromBlock(const FileBlock& block, io::RandomAccessFile* file,
        ...
        ARROW_ASSIGN_OR_RAISE(auto message,
            ReadMessage(
                block.offset, block.metadata_length, file, fields_loader));
        return std::move(message);
    }

    static Future<std::shared_ptr<Message>>
    ReadMessageFromBlockAsync(const FileBlock& block,
        io::RandomAccessFile* file, const io::IOContext& io_context) {
        if (!bit_util::IsMultipleOf8(block.offset)
            || !bit_util::IsMultipleOf8(block.metadata_length)
            || !bit_util::IsMultipleOf8(block.body_length)) {
            return Status::Invalid("Unaligned block in IPC file");
        ...
        return ReadMessageAsync(block.offset, block.metadata_length,
            block.body_length, file, io_context);
    }
    ...
    public:
        ...
        explicit WholeIpcFileRecordBatchGenerator(
            ...
            io::IOContext io_context, arrow::internal::Executor* executor)
        ...
        static Status ReadDictionaries(RecordBatchFileReaderImpl* state,
    ...
    public:
        ...
        int
        num_record_batches() const override {
            return static_cast<int>(
                internal::FlatBuffersVectorSize(footer_->recordBatches()));
        }
        ...
        static Status
        LoadFieldsSubset(const flatbuf::RecordBatch* metadata,
            ...
            for (int i = 0; i < schema->num_fields(); ++i) {
                ...
                if ((inclusion_mask == nullptr) || (*inclusion_mask)[i]) {
                    ...
                    // Skip field. This logic must be executed to advance the
                    // state of the loader to the next field
                    RETURN_NOT_OK(loader.SkipField(&field));
                }
            }
            return Status::OK();
        ...
        ReadRecordBatchAsync(int i) {
            ...
            return Status::Invalid("Asynchronous record batch reading is only "
                                   "supported after a call to "
                                   "PreBufferMetadata or PreBufferBatches");
        }

        Result<std::shared_ptr<RecordBatch>>
        ReadRecordBatch(int i) override {
            ARROW_ASSIGN_OR_RAISE(
                auto batch_with_metadata, ReadRecordBatchWithCustomMetadata(i));
        ...
        ReadRecordBatchWithCustomMetadata(int i) override {
            ...
            if (cached_metadata != cached_metadata_.end()) {
                ...
                ARROW_ASSIGN_OR_RAISE(
                    auto message_obj, cached_metadata->second.result());
                ARROW_ASSIGN_OR_RAISE(
                    auto message, GetFlatbufMessage(message_obj));
                std::shared_ptr<KeyValueMetadata> custom_metadata;
                if (message->custom_metadata() != nullptr) {
                    RETURN_NOT_OK(internal::GetKeyValueMetadata(
                        message->custom_metadata(), &custom_metadata));
            ...
            if (!field_inclusion_mask_.empty()) {
                ...
                fields_loader = [schema, inclusion_mask, read_options](
                                    const void* metadata,
                                    io::RandomAccessFile* file) {
                    return LoadFieldsSubset(
                        static_cast<const flatbuf::RecordBatch*>(metadata),
        ...
        CountRows() override {
            ...
            for (int i = 0; i < num_record_batches(); i++) {
                ...
                const auto* batch = message->header_as_RecordBatch();
                if (batch == nullptr) {
                    return Status::IOError("Header-type of flatbuffer-encoded "
                                           "Message is not RecordBatch.");
                }
                total += batch->length();
            }
            return total;
        ...
        Status
        Open(const std::shared_ptr<io::RandomAccessFile>& file,
        ...
        Status
        Open(io::RandomAccessFile* file, int64_t footer_offset,
            const IpcReadOptions& options) {
            // The metadata_cache_ may have already been constructed with an
            // owned file in the owning overload of Open
            if (!metadata_cache_) {
                metadata_cache_
                    = std::make_shared<io::internal::ReadRangeCache>(file,
                        file->io_context(), options.pre_buffer_cache_options);
            }
            file_ = file;
            options_ = options;
        ...
        Future<>
        OpenAsync(const std::shared_ptr<io::RandomAccessFile>& file,
            int64_t footer_offset, const IpcReadOptions& options) {
            owned_file_ = file;
            metadata_cache_ = std::make_shared<io::internal::ReadRangeCache>(
                file, file->io_context(), options.pre_buffer_cache_options);
        ...
        Future<>
        OpenAsync(io::RandomAccessFile* file, int64_t footer_offset,
            const IpcReadOptions& options) {
            // The metadata_cache_ may have already been constructed with an
            // owned file in the owning overload of OpenAsync
            if (!metadata_cache_) {
                metadata_cache_
                    = std::make_shared<io::internal::ReadRangeCache>(file,
                        file->io_context(), options.pre_buffer_cache_options);
            ...
            auto self = std::dynamic_pointer_cast<RecordBatchFileReaderImpl>(
                shared_from_this());
            return ReadFooterAsync(cpu_executor)
                .Then([self, options]() -> Status {
                    // Get the schema and record any observed dictionaries
                    RETURN_NOT_OK(UnpackSchemaMessage(self->footer_->schema(),
        ...
        GetRecordBatchGenerator(const bool coalesce,
            const io::IOContext& io_context,
            const io::CacheOptions cache_options,
            arrow::internal::Executor* executor) override {
            auto state = std::dynamic_pointer_cast<RecordBatchFileReaderImpl>(
                shared_from_this());
            // Prebuffering causes us to use a lot of futures which, at the
            // moment, can only slow things down when we are doing zero-copy
            // in-memory reads.
            //
            // Prebuffering's read patterns are also slightly worse than the
            // alternative when doing whole-file reads because the logic is not
            // in place to recognize we can just read the entire file up-front
            ...
            if (coalesce && !file_->supports_zero_copy()) {
                if (!owned_file_) {
                    return Status::Invalid(
                        "Cannot coalesce without an owned file");
                }
                // Since the user is asking for all fields then we can cache the
                // entire file (up to the footer)
                cached_source = std::make_shared<io::internal::ReadRangeCache>(
                    file_, io_context, cache_options);
                RETURN_NOT_OK(cached_source->Cache({{0, footer_offset_}}));
            }
            return WholeIpcFileRecordBatchGenerator(std::move(state),
                std::move(cached_source), io_context, executor);
        }

        Status
        DoPreBufferMetadata(const std::vector<int>& indices) {
        ...
        Status
        PreBufferMetadata(const std::vector<int>& indices) override {
            if (indices.empty()) {
    ...
    private:
        ...
        FileBlock
        GetDictionaryBlock(int i) const {
            return FileBlockFromFlatbuffer(footer_->dictionaries()->Get(i));
        }

        Result<std::unique_ptr<Message>>
        ReadMessageFromBlock(const FileBlock& block,
            const FieldsLoaderFunction& fields_loader = {}) {
            ARROW_ASSIGN_OR_RAISE(auto message,
        ...
        Status
        ReadOneDictionary(Message* message, const IpcReadContext& context) {
            CHECK_HAS_BODY(*message);
            ARROW_ASSIGN_OR_RAISE(
                auto reader, Buffer::GetReader(message->body()));
            DictionaryKind kind;
            RETURN_NOT_OK(ReadDictionary(
                *message->metadata(), context, &kind, reader.get()));
            if (kind == DictionaryKind::Replacement) {
                return Status::Invalid(
                    "Unsupported dictionary replacement in IPC file");
            }
            if (kind == DictionaryKind::Delta) {
        ...
        void
        AddDictionaryRanges(std::vector<io::ReadRange>* ranges) const {
        ...
        void
        AddMetadataRanges(const std::vector<int>& indices,
        ...
        Status
        CacheMetadata(const std::vector<int>& indices) {
            std::vector<io::ReadRange> ranges;
        ...
        Future<>
        WaitForMetadatas(const std::vector<int>& indices) {
            std::vector<io::ReadRange> ranges;
            AddMetadataRanges(indices, &ranges);
            return metadata_cache_->WaitFor(std::move(ranges));
        }

        Result<IpcReadContext>
        GetIpcReadContext(const flatbuf::Message* message,
            ...
            if (context.compression == Compression::UNCOMPRESSED
                && message->version() == flatbuf::MetadataVersion::V4) {
                // Possibly obtain codec information from experimental
                // serialization format in 0.17.x
                RETURN_NOT_OK(
                    GetCompressionExperimental(message, &compression));
            }
            context.compression = compression;
            context.metadata_version
                = internal::GetMetadataVersion(message->version());
        ...
        static Result<const flatbuf::RecordBatch*>
        GetBatchFromMessage(const flatbuf::Message* message) {
            const auto* batch = message->header_as_RecordBatch();
            if (batch == nullptr) {
                return Status::IOError("Header-type of flatbuffer-encoded "
                                       "Message is not RecordBatch.");
        ...
        struct CachedRecordBatchReadContext {
            CachedRecordBatchReadContext(std::shared_ptr<Schema> sch,
                ...
                , loader(batch, context.metadata_version, context.options,
                      block_data_offset)
                , columns(schema->num_fields())
                , cache(file, file->io_context(),
                      io::CacheOptions::LazyDefaults())
                , length(batch->length()) {}

            Status
            CalculateLoadRequest() {
                std::shared_ptr<Schema> out_schema;
                RETURN_NOT_OK(GetInclusionMaskAndOutSchema(schema,
                    context.options.included_fields, &inclusion_mask,
                    &out_schema));
                ...
                for (int i = 0; i < schema->num_fields(); ++i) {
                    ...
                    if (inclusion_mask.empty() || inclusion_mask[i]) {
                        ...
                        if (length != column->length) {
                            return Status::IOError("Array length did not match "
                                                   "record batch length");
                        }
                        columns[i] = std::move(column);
                        if (!inclusion_mask.empty()) {
                        ...
                        // Skip field. This logic must be executed to advance
                        // the state of the loader to the next field
                        RETURN_NOT_OK(loader.SkipField(&field));
                    }
                }
                if (!inclusion_mask.empty()) {
            ...
            ReadAsync() {
                RETURN_NOT_OK(
                    cache.Cache(loader.read_request().ranges_to_read()));
            ...
            CreateRecordBatch() {
                ...
                for (const auto& range_to_read :
                    loader.read_request().ranges_to_read()) {
                    ARROW_ASSIGN_OR_RAISE(
                        auto buffer, cache.Read(range_to_read));
                    buffers.push_back(std::move(buffer));
                }
                loader.read_request().FulfillRequest(buffers);

                // Dictionary resolution needs to happen on the unfiltered
                // columns, because fields are mapped structurally (by path in
                // the original schema).
                RETURN_NOT_OK(ResolveDictionaries(columns,
                    *context.dictionary_memo, context.options.memory_pool));
                if (!inclusion_mask.empty()) {
                    columns.clear();
                } else {
                    filtered_columns = std::move(columns);
                ...
                if (context.compression != Compression::UNCOMPRESSED) {
                    RETURN_NOT_OK(DecompressBuffers(context.compression,
                        context.options, &filtered_columns));
                }

                // swap endian in a set of ArrayData if necessary (swap_endian
                // == true)
                if (context.swap_endian) {
                    for (auto& filtered_column : filtered_columns) {
                        ARROW_ASSIGN_OR_RAISE(filtered_column,
        ...
        ReadCachedRecordBatch(
            int index, const Future<std::shared_ptr<Message>>& message_fut) {
            stats_.num_record_batches.fetch_add(1, std::memory_order_relaxed);
            return dictionary_load_finished_
                ...
                .Then([this, index](const std::shared_ptr<Message>& message_obj)
                    ...
                    ARROW_ASSIGN_OR_RAISE(
                        auto message, GetFlatbufMessage(message_obj));
                    ARROW_ASSIGN_OR_RAISE(
                        auto batch, GetBatchFromMessage(message));
                    ARROW_ASSIGN_OR_RAISE(
                        auto context, GetIpcReadContext(message, batch));

                    auto read_context
                        = std::make_shared<CachedRecordBatchReadContext>(
                            schema_, batch, context, file_, owned_file_,
                            block.offset
                                + static_cast<int64_t>(block.metadata_length));
                    RETURN_NOT_OK(read_context->CalculateLoadRequest());
                    return read_context->ReadAsync().Then([read_context] {
                        return read_context->CreateRecordBatch();
                    });
                });
        }
        ...
        Future<>
        ReadFooterAsync(arrow::internal::Executor* executor) {
            const auto magic_size
                = static_cast<std::int64_t>(strlen(kArrowMagicBytes));
            ...
            auto self = std::dynamic_pointer_cast<RecordBatchFileReaderImpl>(
                shared_from_this());
            auto read_magic = file_->ReadAsync(
                footer_offset_ - file_end_size, file_end_size);
            if (executor != nullptr) {
                read_magic = executor->Transfer(std::move(read_magic));
            }
            return read_magic
                .Then([=](const std::shared_ptr<Buffer>& buffer)
                          -> Future<std::shared_ptr<Buffer>> {
                    const int64_t expected_footer_size
                        = magic_size + sizeof(int32_t);
                    if (buffer->size() < expected_footer_size) {
                        return Status::Invalid("Unable to read ",
                            expected_footer_size, "from end of file");
                    }

                    if (memcmp(buffer->data() + sizeof(int32_t),
                            kArrowMagicBytes, magic_size)
                        != 0) {
                    ...
                    if (footer_length <= 0
                        || footer_length
                            > self->footer_offset_ - magic_size * 2 - 4) {
                        return Status::Invalid(
                ...
                .Then([=](const std::shared_ptr<Buffer>& buffer) -> Status {
                    ...
                    if (!internal::VerifyFlatbuffers<flatbuf::Footer>(
                            data, size)) {
                        return Status::IOError(
                            "Verification of flatbuffer-encoded Footer "
                            "failed.");
                    }
                    self->footer_ = flatbuf::GetFooter(data);

                    const auto* fb_metadata = self->footer_->custom_metadata();
                    if (fb_metadata != nullptr) {
                        std::shared_ptr<KeyValueMetadata> md;
                        RETURN_NOT_OK(
                            internal::GetKeyValueMetadata(fb_metadata, &md));
        ...
        int
        num_dictionaries() const {
            return static_cast<int>(
                internal::FlatBuffersVectorSize(footer_->dictionaries()));
        }
        ...
        // The location where the Arrow file layout ends. May be the end of the
        // file or some other location if embedded in a larger file.
        int64_t footer_offset_{0};

        // Footer metadata
        std::shared_ptr<Buffer> footer_buffer_;
        const flatbuf::Footer* footer_{NULLPTR};
        ...
        std::unordered_map<int, Future<std::shared_ptr<Message>>>
            cached_metadata_;
    ...
    Result<std::shared_ptr<RecordBatchFileReader>>
    RecordBatchFileReader::Open(io::RandomAccessFile* file,
        int64_t footer_offset, const IpcReadOptions& options) {
    ...
    RecordBatchFileReader::OpenAsync(
        ...
        return result->OpenAsync(file, footer_offset, options)
            .Then([=]() -> Result<std::shared_ptr<RecordBatchFileReader>> {
                return result;
            });
    }

    Future<std::shared_ptr<RecordBatchFileReader>>
    RecordBatchFileReader::OpenAsync(io::RandomAccessFile* file,
        int64_t footer_offset, const IpcReadOptions& options) {
        auto result = std::make_shared<RecordBatchFileReaderImpl>();
        return result->OpenAsync(file, footer_offset, options)
            .Then([=]() -> Result<std::shared_ptr<RecordBatchFileReader>> {
    ...
    WholeIpcFileRecordBatchGenerator::operator()() {
        ...
        auto block = FileBlockFromFlatbuffer(
            state->footer_->recordBatches()->Get(index_++));
        auto read_message = ReadBlock(block);
        auto read_messages = read_dictionaries_.Then(
            [read_message]() { return read_message; });
        // Force transfer. This may be wasteful in some cases, but ensures we
        // get off the I/O threads as soon as possible, and ensures we don't
        // decode record batches synchronously in the case that the message read
        ...
        return read_messages.Then(
            [=](const std::shared_ptr<Message>& message) -> Result<Item> {
                return ReadRecordBatch(state.get(), message.get());
            });
    }

    Future<std::shared_ptr<Message>>
    WholeIpcFileRecordBatchGenerator::ReadBlock(const FileBlock& block) {
        if (cached_source_) {
            ...
            return cached_source->WaitFor({range}).Then(
                [cached_source, pool,
                    range]() -> Result<std::shared_ptr<Message>> {
                    ARROW_ASSIGN_OR_RAISE(
                        auto buffer, cached_source->Read(range));
    ...
    Status
    WholeIpcFileRecordBatchGenerator::ReadDictionaries(
        RecordBatchFileReaderImpl* state,
        const std::vector<std::shared_ptr<Message>>& dictionary_messages) {
        IpcReadContext context(
            &state->dictionary_memo_, state->options_, state->swap_endian_);
    ...
    Result<std::shared_ptr<RecordBatch>>
    WholeIpcFileRecordBatchGenerator::ReadRecordBatch(
        RecordBatchFileReaderImpl* state, Message* message) {
        CHECK_HAS_BODY(*message);
        ARROW_ASSIGN_OR_RAISE(auto reader, Buffer::GetReader(message->body()));
        IpcReadContext context(
    ...
    Status
    Listener::OnSchemaDecoded(std::shared_ptr<Schema> schema) {
        return Status::OK();
    }

    Status
    Listener::OnRecordBatchDecoded(std::shared_ptr<RecordBatch> record_batch) {
        return Status::NotImplemented(
            "OnRecordBatchDecoded() callback isn't implemented");
    }
    ...
    public:
        explicit StreamDecoderImpl(
            std::shared_ptr<Listener> listener, IpcReadOptions options)
        ...
        Status
        OnMessageDecoded(std::unique_ptr<Message> message) override {
            ++stats_.num_messages;
            switch (state_) {
                case State::SCHEMA:
                    ARROW_RETURN_NOT_OK(
                        OnSchemaMessageDecoded(std::move(message)));
                    break;
                case State::INITIAL_DICTIONARIES:
                    ARROW_RETURN_NOT_OK(
                        OnInitialDictionaryMessageDecoded(std::move(message)));
                    break;
                case State::RECORD_BATCHES:
                    ARROW_RETURN_NOT_OK(
                        OnRecordBatchMessageDecoded(std::move(message)));
                    break;
                case State::EOS:
                    break;
            }
        ...
        Status
        Consume(const uint8_t* data, int64_t size) {
            return message_decoder_.Consume(data, size);
        }

        Status
        Consume(std::shared_ptr<Buffer> buffer) {
        ...
        [[nodiscard]] int64_t
        next_required_size() const {
            return message_decoder_.next_required_size();
        }
    ...
    private:
        Status
        OnSchemaMessageDecoded(std::unique_ptr<Message> message) {
            RETURN_NOT_OK(UnpackSchemaMessage(*message, options_,
                &dictionary_memo_, &schema_, &out_schema_,
                &field_inclusion_mask_, &swap_endian_));
        ...
        Status
        OnInitialDictionaryMessageDecoded(std::unique_ptr<Message> message) {
            if (message->type() != MessageType::DICTIONARY_BATCH) {
                return Status::Invalid(
                    "IPC stream did not have the expected number (",
        ...
        Status
        OnRecordBatchMessageDecoded(std::unique_ptr<Message> message) {
            if (message->type() == MessageType::DICTIONARY_BATCH) {
            ...
            ARROW_ASSIGN_OR_RAISE(auto batch_with_metadata,
                ReadRecordBatchInternal(*message->metadata(), schema_,
                    field_inclusion_mask_, context, reader.get()));
            ++stats_.num_record_batches;
            return listener_->OnRecordBatchDecoded(
                std::move(batch_with_metadata.batch));
        }
        ...
        State state_{State::SCHEMA};
        MessageDecoder message_decoder_;
        std::vector<bool> field_inclusion_mask_;
        int n_required_dictionaries_{0};
        DictionaryMemo dictionary_memo_;
        std::shared_ptr<Schema> schema_, out_schema_;
        ReadStats stats_;
        bool swap_endian_;
    ...
    StreamDecoder::StreamDecoder(
        std::shared_ptr<Listener> listener, IpcReadOptions options) {
        impl_ = std::make_unique<StreamDecoderImpl>(
            std::move(listener), std::move(options));
    }

    StreamDecoder::~StreamDecoder() = default;

    Status
    StreamDecoder::Consume(const uint8_t* data, int64_t size) {
        return impl_->Consume(data, size);
    }
    Status
    StreamDecoder::Consume(std::shared_ptr<Buffer> buffer) {
        return impl_->Consume(std::move(buffer));
    }

    std::shared_ptr<Schema>
    StreamDecoder::schema() const {
        return impl_->schema();
    }

    int64_t
    StreamDecoder::next_required_size() const {
        return impl_->next_required_size();
    }

    ReadStats
    StreamDecoder::stats() const {
        return impl_->stats();
    }

    Result<std::shared_ptr<Schema>>
    ReadSchema(io::InputStream* stream, DictionaryMemo* dictionary_memo) {
        std::unique_ptr<MessageReader> reader = MessageReader::Open(stream);
        ARROW_ASSIGN_OR_RAISE(
            std::unique_ptr<Message> message, reader->ReadNextMessage());
        if (!message) {
            return Status::Invalid(
                "Tried reading schema message, was null or length 0");
        }
    ...
    Result<std::shared_ptr<Schema>>
    ReadSchema(const Message& message, DictionaryMemo* dictionary_memo) {
        std::shared_ptr<Schema> result;
        RETURN_NOT_OK(
            internal::GetSchema(message.header(), dictionary_memo, &result));
        return result;
    }

    Result<std::shared_ptr<Tensor>>
    ReadTensor(io::InputStream* file) {
        std::unique_ptr<Message> message;
        RETURN_NOT_OK(ReadContiguousPayload(file, &message));
    ...
    Result<std::shared_ptr<Tensor>>
    ReadTensor(const Message& message) {
        std::shared_ptr<DataType> type;
        std::vector<int64_t> shape;
        std::vector<int64_t> strides;
        std::vector<std::string> dim_names;
        CHECK_HAS_BODY(message);
        RETURN_NOT_OK(internal::GetTensorMetadata(
            *message.metadata(), &type, &shape, &strides, &dim_names));
        return Tensor::Make(type, message.body(), shape, strides, dim_names);
    }

    namespace {

        Result<std::shared_ptr<SparseIndex>>
        ReadSparseCOOIndex(const flatbuf::SparseTensor* sparse_tensor,
            const std::vector<int64_t>& shape, int64_t non_zero_length,
            io::RandomAccessFile* file) {
            const auto* sparse_index
                = sparse_tensor->sparseIndex_as_SparseTensorIndexCOO();
            ...
            const auto* indices_buffer = sparse_index->indicesBuffer();
            ARROW_ASSIGN_OR_RAISE(auto indices_data,
                file->ReadAt(
                    indices_buffer->offset(), indices_buffer->length()));
            std::vector<int64_t> indices_shape({non_zero_length, ndim});
            const auto* indices_strides = sparse_index->indicesStrides();
            std::vector<int64_t> strides(2);
            if ((indices_strides != nullptr) && indices_strides->size() > 0) {
                if (indices_strides->size() != 2) {
                    return Status::Invalid(
                        "Wrong size for indicesStrides in SparseCOOIndex");
                }
            ...
            return SparseCOOIndex::Make(
                std::make_shared<Tensor>(
                    indices_type, indices_data, indices_shape, strides),
                sparse_index->isCanonical());
        }

        Result<std::shared_ptr<SparseIndex>>
        ReadSparseCSXIndex(const flatbuf::SparseTensor* sparse_tensor,
            const std::vector<int64_t>& shape, int64_t non_zero_length,
            io::RandomAccessFile* file) {
            if (shape.size() != 2) {
                return Status::Invalid(
            ...
            const auto* sparse_index
                = sparse_tensor->sparseIndex_as_SparseMatrixIndexCSX();
            ...
            const auto* indptr_buffer = sparse_index->indptrBuffer();
            ...
            switch (sparse_index->compressedAxis()) {
                case flatbuf::SparseMatrixCompressedAxis::Row: {
                    ...
                    const int64_t indptr_minimum_bytes
                        = indptr_shape[0] * indptr_byte_width;
                    if (indptr_minimum_bytes > indptr_buffer->length()) {
                        return Status::Invalid("shape is inconsistent to the "
                                               "size of indptr buffer");
                    }
                    return std::make_shared<SparseCSRIndex>(
                        std::make_shared<Tensor>(
                            indptr_type, indptr_data, indptr_shape),
                        std::make_shared<Tensor>(
                            indices_type, indices_data, indices_shape));
                }
                ...
                default:
                    return Status::Invalid(
                        "Invalid value of SparseMatrixCompressedAxis");
            }
        }

        Result<std::shared_ptr<SparseIndex>>
        ReadSparseCSFIndex(const flatbuf::SparseTensor* sparse_tensor,
            const std::vector<int64_t>& shape, io::RandomAccessFile* file) {
            const auto* sparse_index
                = sparse_tensor->sparseIndex_as_SparseTensorIndexCSF();
            const auto ndim = static_cast<int64_t>(shape.size());
            const auto* indptr_buffers = sparse_index->indptrBuffers();
            const auto* indices_buffers = sparse_index->indicesBuffers();
            ...
            RETURN_NOT_OK(internal::GetSparseCSFIndexMetadata(sparse_index,
                &axis_order, &indices_size, &indptr_type, &indices_type));
            ...
            for (int i = 0; i < static_cast<int>(indices_buffers->size());
                 ++i) {
        ...
        MakeSparseTensorWithSparseCOOIndex(
            ...
            const std::shared_ptr<SparseCOOIndex>& sparse_index,
            int64_t non_zero_length, const std::shared_ptr<Buffer>& data) {
            return SparseCOOTensor::Make(
                sparse_index, type, data, shape, dim_names);
        }
        ...
        MakeSparseTensorWithSparseCSRIndex(
            ...
            const std::vector<int64_t>& shape,
            const std::vector<std::string>& dim_names,
            const std::shared_ptr<SparseCSRIndex>& sparse_index,
            int64_t non_zero_length, const std::shared_ptr<Buffer>& data) {
            return SparseCSRMatrix::Make(
                sparse_index, type, data, shape, dim_names);
        }
        ...
        MakeSparseTensorWithSparseCSCIndex(
            ...
            const std::vector<int64_t>& shape,
            const std::vector<std::string>& dim_names,
            const std::shared_ptr<SparseCSCIndex>& sparse_index,
            int64_t non_zero_length, const std::shared_ptr<Buffer>& data) {
            return SparseCSCMatrix::Make(
                sparse_index, type, data, shape, dim_names);
        }

        Result<std::shared_ptr<SparseTensor>>
        MakeSparseTensorWithSparseCSFIndex(
            const std::shared_ptr<DataType>& type,
            const std::vector<int64_t>& shape,
            const std::vector<std::string>& dim_names,
            const std::shared_ptr<SparseCSFIndex>& sparse_index,
            const std::shared_ptr<Buffer>& data) {
            return SparseCSFTensor::Make(
                sparse_index, type, data, shape, dim_names);
        }

        Status
        ReadSparseTensorMetadata(const Buffer& metadata,
            ...
            RETURN_NOT_OK(internal::GetSparseTensorMetadata(metadata, out_type,
                out_shape, out_dim_names, out_non_zero_length, out_format_id));

            const flatbuf::Message* message = nullptr;
            RETURN_NOT_OK(internal::VerifyMessage(
                metadata.data(), metadata.size(), &message));

            const auto* sparse_tensor = message->header_as_SparseTensor();
            if (sparse_tensor == nullptr) {
                return Status::IOError("Header-type of flatbuffer-encoded "
                                       "Message is not SparseTensor.");
            }
            *out_fb_sparse_tensor = sparse_tensor;

            const auto* buffer = sparse_tensor->data();
            if (!bit_util::IsMultipleOf8(buffer->offset())) {
                return Status::Invalid("Buffer of sparse index data did not "
                                       "start on 8-byte aligned offset: ",
                    buffer->offset());
            }
            *out_buffer = buffer;
    ...
    namespace internal {
        ...
        namespace {
            ...
            Result<size_t>
            GetSparseTensorBodyBufferCount(
                SparseTensorFormat::type format_id, const size_t ndim) {
                switch (format_id) {
                    ...
                    default:
                        return Status::Invalid(
                            "Unrecognized sparse tensor format");
                }
            }

            Status
            CheckSparseTensorBodyBufferCount(const IpcPayload& payload,
                SparseTensorFormat::type sparse_tensor_format_id,
                const size_t ndim) {
                size_t expected_body_buffer_count = 0;
                ARROW_ASSIGN_OR_RAISE(expected_body_buffer_count,
                    GetSparseTensorBodyBufferCount(
                        sparse_tensor_format_id, ndim));
                if (payload.body_buffers.size() != expected_body_buffer_count) {
                    return Status::Invalid(
                        "Invalid body buffer count for a sparse tensor");
                }
        ...
        ReadSparseTensorBodyBufferCount(const Buffer& metadata) {
            ...
            RETURN_NOT_OK(internal::GetSparseTensorMetadata(
                metadata, nullptr, &shape, nullptr, nullptr, &format_id));

            return GetSparseTensorBodyBufferCount(
                format_id, static_cast<size_t>(shape.size()));
        }

        Result<std::shared_ptr<SparseTensor>>
        ReadSparseTensorPayload(const IpcPayload& payload) {
            std::shared_ptr<DataType> type;
            std::vector<int64_t> shape;
            std::vector<std::string> dim_names;
            ...
            RETURN_NOT_OK(ReadSparseTensorMetadata(*payload.metadata, &type,
                &shape, &dim_names, &non_zero_length, &sparse_tensor_format_id,
                &sparse_tensor, &buffer));

            RETURN_NOT_OK(CheckSparseTensorBodyBufferCount(payload,
                sparse_tensor_format_id, static_cast<size_t>(shape.size())));
            ...
            switch (sparse_tensor_format_id) {
                case SparseTensorFormat::COO: {
                    ...
                    ARROW_ASSIGN_OR_RAISE(sparse_index,
                        SparseCOOIndex::Make(indices_type, shape,
                            non_zero_length, payload.body_buffers[0]));
                    return MakeSparseTensorWithSparseCOOIndex(type, shape,
                        dim_names, sparse_index, non_zero_length,
                ...
                case SparseTensorFormat::CSR: {
                    ...
                    ARROW_ASSIGN_OR_RAISE(sparse_index,
                        SparseCSRIndex::Make(indices_type, shape,
                            non_zero_length, payload.body_buffers[0],
                            payload.body_buffers[1]));
                    return MakeSparseTensorWithSparseCSRIndex(type, shape,
                        dim_names, sparse_index, non_zero_length,
                        payload.body_buffers[2]);
                }
                case SparseTensorFormat::CSC: {
                    ...
                    ARROW_CHECK_EQ(indptr_type, indices_type);
                    ARROW_ASSIGN_OR_RAISE(sparse_index,
                        SparseCSCIndex::Make(indices_type, shape,
                            non_zero_length, payload.body_buffers[0],
                            payload.body_buffers[1]));
                    return MakeSparseTensorWithSparseCSCIndex(type, shape,
                        dim_names, sparse_index, non_zero_length,
                        payload.body_buffers[2]);
                }
                case SparseTensorFormat::CSF: {
                    ...
                    RETURN_NOT_OK(internal::GetSparseCSFIndexMetadata(
                        sparse_tensor->sparseIndex_as_SparseTensorIndexCSF(),
                        &axis_order, &indices_size, &indptr_type,
                        &indices_type));
                    ARROW_CHECK_EQ(indptr_type, indices_type);
                    ...
                    ARROW_ASSIGN_OR_RAISE(sparse_index,
                        SparseCSFIndex::Make(indptr_type, indices_type,
                            indices_size, axis_order, indptr_data,
                            indices_data));
                    return MakeSparseTensorWithSparseCSFIndex(type, shape,
                        dim_names, sparse_index,
    ...
    Result<std::shared_ptr<SparseTensor>>
    ReadSparseTensor(const Buffer& metadata, io::RandomAccessFile* file) {
        std::shared_ptr<DataType> type;
        std::vector<int64_t> shape;
        std::vector<std::string> dim_names;
        int64_t non_zero_length;
        ...
        RETURN_NOT_OK(ReadSparseTensorMetadata(metadata, &type, &shape,
            &dim_names, &non_zero_length, &sparse_tensor_format_id,
            &sparse_tensor, &buffer));

        ARROW_ASSIGN_OR_RAISE(
            auto data, file->ReadAt(buffer->offset(), buffer->length()));
        ...
        switch (sparse_tensor_format_id) {
            case SparseTensorFormat::COO: {
                ARROW_ASSIGN_OR_RAISE(sparse_index,
                    ReadSparseCOOIndex(
                        sparse_tensor, shape, non_zero_length, file));
                return MakeSparseTensorWithSparseCOOIndex(type, shape,
                    dim_names,
                    checked_pointer_cast<SparseCOOIndex>(sparse_index),
            ...
            case SparseTensorFormat::CSR: {
                ...
                return MakeSparseTensorWithSparseCSRIndex(type, shape,
                    dim_names,
                    checked_pointer_cast<SparseCSRIndex>(sparse_index),
                    non_zero_length, data);
            }
            case SparseTensorFormat::CSC: {
                ARROW_ASSIGN_OR_RAISE(sparse_index,
                ...
                return MakeSparseTensorWithSparseCSCIndex(type, shape,
                    dim_names,
                    checked_pointer_cast<SparseCSCIndex>(sparse_index),
                    non_zero_length, data);
            }
            case SparseTensorFormat::CSF: {
                ARROW_ASSIGN_OR_RAISE(sparse_index,
                    ReadSparseCSFIndex(sparse_tensor, shape, file));
                return MakeSparseTensorWithSparseCSFIndex(type, shape,
                    dim_names,
                    checked_pointer_cast<SparseCSFIndex>(sparse_index), data);
            }
            default:
                return Status::Invalid("Unsupported sparse index format");
        }
    ...
    Result<std::shared_ptr<SparseTensor>>
    ReadSparseTensor(const Message& message) {
        CHECK_HAS_BODY(message);
        ARROW_ASSIGN_OR_RAISE(auto reader, Buffer::GetReader(message.body()));
    ...
    namespace internal {
        namespace {
            ...
            Status
            ValidateFuzzBatch(const RecordBatch& batch) {
        ...
        Status
        FuzzIpcStream(const uint8_t* data, int64_t size) {
            auto buffer = std::make_shared<Buffer>(data, size);
            io::BufferReader buffer_reader(buffer);

            std::shared_ptr<RecordBatchReader> batch_reader;
            ARROW_ASSIGN_OR_RAISE(
                batch_reader, RecordBatchStreamReader::Open(&buffer_reader));
            Status st;
        ...
        Status
        FuzzIpcFile(const uint8_t* data, int64_t size) {
            auto buffer = std::make_shared<Buffer>(data, size);
            io::BufferReader buffer_reader(buffer);

            std::shared_ptr<RecordBatchFileReader> batch_reader;
            ARROW_ASSIGN_OR_RAISE(
                batch_reader, RecordBatchFileReader::Open(&buffer_reader));
            Status st;

            const int n_batches = batch_reader->num_record_batches();
            for (int i = 0; i < n_batches; ++i) {
                ARROW_ASSIGN_OR_RAISE(
                    auto batch, batch_reader->ReadRecordBatch(i));
                st &= ValidateFuzzBatch(*batch);
            }
        ...
        Status
        FuzzIpcTensorStream(const uint8_t* data, int64_t size) {
            auto buffer = std::make_shared<Buffer>(data, size);
            io::BufferReader buffer_reader(buffer);
        ...
        Result<int64_t>
        IoRecordedRandomAccessFile::GetSize() {
            return file_size_;
        }

        Result<int64_t>
        IoRecordedRandomAccessFile::ReadAt(
            int64_t position, int64_t nbytes, void* out) {
            auto num_bytes_read
                = std::min(file_size_, position + nbytes) - position;

            if (!read_ranges_.empty()
                && position
                    == read_ranges_.back().offset
                        + read_ranges_.back().length) {
        ...
        Result<std::shared_ptr<Buffer>>
        IoRecordedRandomAccessFile::ReadAt(int64_t position, int64_t nbytes) {
        ...
        Status
        IoRecordedRandomAccessFile::Close() {
            closed_ = true;
            return Status::OK();
        }

        Status
        IoRecordedRandomAccessFile::Abort() {
            return Status::OK();
        }

        Result<int64_t>
        IoRecordedRandomAccessFile::Tell() const {
            return position_;
        }

        bool
        IoRecordedRandomAccessFile::closed() const {
            return closed_;
        }

        Status
        IoRecordedRandomAccessFile::Seek(int64_t position) {
            position_ = position;
            return Status::OK();
        }

        Result<int64_t>
        IoRecordedRandomAccessFile::Read(int64_t nbytes, void* out) {
            ARROW_ASSIGN_OR_RAISE(
                int64_t bytes_read, ReadAt(position_, nbytes, out));
            position_ += bytes_read;
            return bytes_read;
        }

        Result<std::shared_ptr<Buffer>>
        IoRecordedRandomAccessFile::Read(int64_t nbytes) {
            ARROW_ASSIGN_OR_RAISE(
                std::shared_ptr<Buffer> buffer, ReadAt(position_, nbytes));
            auto num_bytes_read
                = std::min(file_size_, position_ + nbytes) - position_;
            position_ += num_bytes_read;
            return std::move(buffer);
        }

        const io::IOContext&
        IoRecordedRandomAccessFile::io_context() const {
            return io_context_;
        }

        const std::vector<io::ReadRange>&
        IoRecordedRandomAccessFile::GetReadRanges() const {
            return read_ranges_;
        }
