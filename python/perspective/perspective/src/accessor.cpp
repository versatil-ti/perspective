...
get_column_names(const t_val& data, std::int32_t format) {
    std::vector<std::string> names;
    if (format == 0) {
        // record
        auto data_list = data.cast<py::list>();
        std::int32_t max_check = 50;

        if (!data_list.empty() != 0u) {
        ...
        for (auto ix = 0; ix < check_index; ix++) {
            auto next_dict = data_list[ix].cast<py::dict>();
        ...
        auto data_dict = data.cast<py::dict>();
...
t_dtype
infer_type(t_val x, const t_val& date_validator) {
    auto type_string = x.get_type().attr("__name__").cast<std::string>();
    ...
    if (x.is_none()) {
        ...
        auto parsed_type = date_validator.attr("format")(x).cast<t_dtype>();
        if (parsed_type == t_dtype::DTYPE_DATE
            || parsed_type == t_dtype::DTYPE_TIME) {
            t = parsed_type;
        } else {
            auto lower = x.attr("lower")().cast<std::string>();
...
t_dtype
get_data_type(const t_val& data, std::int32_t format, const py::str& name,
    const t_val& date_validator) {
    std::int32_t i = 0;
    ...
    if (format == 0) {
        ...
        while (
            !inferredType.is_initialized() && i < 100 && i < data_list.size()) {
...
std::vector<t_dtype>
get_data_types(const t_val& data, std::int32_t format,
    std::vector<std::string> names, const t_val& date_validator) {
    ...
    if (format == 2) {
        ...
        for (auto tup : data_dict) {
            ...
            if (data_type == "type") {
                value = py::str(tup.second.cast<py::object>().attr("__name__"))
                            .cast<std::string>();
            } else {
    ...
    for (auto i = 0; i < names.size(); ++i) {
        t_dtype type
            = get_data_type(data, format, py::str(names[i]), date_validator);
        types[i] = type;
    }
