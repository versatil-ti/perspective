...
_make_table_py(std::shared_ptr<Table> tbl,
    const std::shared_ptr<t_gnode>& gnode, const std::shared_ptr<t_pool>& pool,
    std::uint32_t offset, const t_data_accessor& accessor, std::uint32_t limit,
    ...
    if (is_arrow && !is_delete) {
        ...
        if (is_csv) {
            ...
            auto bytes = accessor.cast<py::bytes>();
        ...
        auto format = accessor.attr("format")().cast<std::int32_t>();
    ...
    make_table_py(t_data_accessor accessor, std::uint32_t limit,
        ...
        return _make_table_py(tbl, gnode, pool, 0, std::move(accessor), limit,
            std::move(index), op, false, is_arrow, is_csv, port_id);
    }

    std::shared_ptr<Table>
    update_table_py(const t_val& table, t_data_accessor accessor,
        std::uint32_t limit, std::string index, t_op op, bool is_arrow,
        bool is_csv, t_uindex port_id) {

        auto tbl = table.cast<std::shared_ptr<Table>>();
        ...
        return _make_table_py(tbl, gnode, pool, offset, std::move(accessor),
            limit, std::move(index), op, true, is_arrow, is_csv, port_id);
    }
